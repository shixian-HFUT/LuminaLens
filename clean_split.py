...
def clean_and_split(opt):
    df = read_ava_txt(r"F:\finalwork\dataset\AVA_dataset\AVA.txt")
    df = remove_all_not_found_image(df, opt.path_to_images)

    df_train, df_val_test = train_test_split(df, train_size=0.0)
    df_val, df_test = train_test_split(df_val_test, train_size=0.0)
